...
namespace vzt
{
	...
	class Mesh
	{
	  public:
		...
		Mesh(std::vector<vzt::Vertex> vertices, std::vector<uint32_t> vertexIndices, const vzt::Material &material);
		Mesh(
		    std::vector<vzt::SubMesh> subMeshes, std::vector<vzt::Vertex> vertices,
		...
		std::vector<vzt::Vertex> CVertices() const
		{
			return m_vertices;
		};
		std::vector<vzt::Material> &Materials()
		...
		std::vector<vzt::Material> CMaterials() const
		{
			return m_materials;
		};
