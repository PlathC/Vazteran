...
namespace vzt {
        ...
        for (auto &shader : graphicPipeline->Shaders()) {
            ...
            if (samplerDescriptorSets.size() == 3) {
                for (std::size_t materialId = 0; materialId < materials.size(); materialId++) {
                    ...
                    if (vkCreateDescriptorPool(logicalDevice->VkHandle(), &descriptorPoolInfo, nullptr,
                                               &m_descriptorPools[materialId]) != VK_SUCCESS) {
                        throw std::runtime_error("Failed to create descriptor pool!");
                    }

                    auto layouts = std::vector<VkDescriptorSetLayout>(m_imageCount,
                                                                      graphicPipeline->DescriptorSetLayout());
                    ...
                    if (vkAllocateDescriptorSets(logicalDevice->VkHandle(), &descriptorSetAllocInfo,
                    ...
                    auto materialInfo = vzt::MaterialInfo{materials[materialId].ambientColor,
                                                          materials[materialId].diffuseColor,
                                                          materials[materialId].specularColor,
                                                          materials[materialId].shininess};
    ...
    void
    RenderObject::Render(VkCommandBuffer commandBuffer, vzt::GraphicPipeline *graphicPipeline, uint32_t imageCount) {
        // TODO: Use this to provide a graphic pipeline suited to the current Shader
        ...
        for (const auto &subMesh: m_subMeshData) {
            vkCmdBindIndexBuffer(commandBuffer, m_subMeshesIndexBuffer->VkHandle(),
                                 subMesh.minOffset * sizeof(uint32_t), VK_INDEX_TYPE_UINT32);

            const auto &currentDescriptorSets = m_materialData[subMesh.materialDataIndex]->descriptorSets;
            vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, graphicPipeline->Layout(), 0, 1,
    ...
    void RenderObject::UpdateDescriptorSet(VkDescriptorSet descriptorSet, VkBuffer uniformBuffer,
        ...
        vkUpdateDescriptorSets(m_logicalDevice->VkHandle(), static_cast<uint32_t>(descriptorWrites.size()),
