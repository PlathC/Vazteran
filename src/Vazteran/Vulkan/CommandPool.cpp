...
namespace vzt {
    CommandPool::CommandPool(LogicalDevice* logicalDevice, uint32_t bufferCount, vzt::RenderFunction renderFunction):
        m_logicalDevice(logicalDevice), m_bufferCount(bufferCount), m_renderFunction(std::move(renderFunction)) {
        ...
        if (vkCreateCommandPool(m_logicalDevice->VkHandle(), &commandPoolInfo, nullptr, &m_vkHandle)
            != VK_SUCCESS) {
    ...
    void CommandPool::AllocateCommandBuffers(uint32_t count) {
        if(!m_commandBuffers.empty()) {
            vkFreeCommandBuffers(m_logicalDevice->VkHandle(), m_vkHandle, static_cast<uint32_t>(m_commandBuffers.size()), m_commandBuffers.data());
        ...
        if (vkAllocateCommandBuffers(m_logicalDevice->VkHandle(), &commandBufferAllocInfo, m_commandBuffers.data()) != VK_SUCCESS) {
            throw std::runtime_error("Failed to allocate command buffers!");
        }
    }

    void CommandPool::SetRenderFunction(vzt::RenderFunction renderFunction) {
        m_renderFunction = std::move(renderFunction);
    }

    void CommandPool::RecordBuffer(const VkExtent2D& swapChainExtent, vzt::GraphicPipeline* graphicPipeline,
        ...
        vkCmdBeginRenderPass(m_commandBuffers[bufferNumber], &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
            if (m_renderFunction)
                m_renderFunction(m_commandBuffers[bufferNumber], graphicPipeline, bufferNumber);
