...
namespace vzt {
    SwapChain::SwapChain(vzt::LogicalDevice* logicalDevice, std::vector<std::unique_ptr<vzt::GraphicPipeline>> graphicPipelines, 
        VkSurfaceKHR surface, vzt::Size2D<int> frameBufferSize) : m_frameBufferSize(std::move(frameBufferSize)), 
            m_surface(surface), m_logicalDevice(logicalDevice), m_graphicPipelines(std::move(graphicPipelines)), 
    ...
    void SwapChain::CreateSwapChain() {
        SwapChainSupportDetails swapChainSupport = m_logicalDevice->ChosenPhysicalDevice()->QuerySwapChainSupport(m_surface);
        ...
        for (const auto& pipeline : m_graphicPipelines) {
            pipeline->Configure(PipelineSettings{
    ...
    void SwapChain::CreateRenderSupport() {
        ...
        for (std::size_t i = 0; i < m_imageCount; i++) {
            ...
            auto frameBuffer = vzt::FrameBuffer(m_logicalDevice, swapChainImages[i], handle, m_graphicPipelines[m_graphicPipelines.size() -1]->RenderPass()->VkHandle(),
