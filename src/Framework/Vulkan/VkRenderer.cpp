...
namespace vzt
{
	      ...
	      m_logicalDevice(std::make_unique<vzt::LogicalDevice>(instance, m_physicalDevice.get(), m_surface))
	{
		...
		/*auto uiPipeline = std::make_unique<vzt::GraphicPipeline>(m_logicalDevice.get(), [&](VkCommandBuffer
		commandBuffer, const vzt::GraphicPipeline* graphicPipeline, uint32_t imageCount) { m_uiRenderer->Update();
		});

		uiPipeline->Create();
		graphicPipelines.emplace_back(std::move(uiPipeline));

		m_uiRenderer = std::make_unique<vzt::UiRenderer>(window, instance, m_logicalDevice.get(),
		uiPipeline->RenderPass());
		*/
		m_swapChain =
		    std::make_unique<vzt::SwapChain>(m_logicalDevice.get(), std::move(graphicPipelines), surface, size);
		for (auto &model : models)
		{
			auto renderObject = std::make_unique<vzt::RenderObject>(
			    m_logicalDevice.get(), m_swapChain->Pipeline(), model, m_swapChain->ImageCount());
